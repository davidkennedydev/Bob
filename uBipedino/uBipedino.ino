...
int RFcenter = 80 + RF_OFFSET;    // variable to store the center servo position 
int LLcenter = 70;
int RLcenter = 84;
int LFcenter = 75;
...
void dance() {
  repeat(4, leftStep, 300);
  CenterServos();
  //repeat(2, leftTap, 200);
  //CenterServos();
  //repeat(2, rightTap, 200);
  //repeat(4, rightStep, 300);
}
...
void rightStep() {
  ...
  //tilt right up
  for (int i=0; i<=ang; i+=stepPrecision){
    Rfoot.write(LFcenter-i);
    delay(sp);
  }
  for (int i=ang; i>=0; i-=stepBackPrecision){
    Rfoot.write(LFcenter-i);
    delay(sp);
  }
}
